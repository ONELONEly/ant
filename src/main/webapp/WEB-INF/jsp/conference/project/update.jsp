...
<div class="x-nav">
    <span class="layui-breadcrumb">
        ...
        <a href="javascript:location.replace(location.href);"><cite style="cursor: pointer;">修改项目</cite></a>
...
<div class="x-body layui-container">
    <form class="layui-form layui-form-pane" enctype="multipart/form-data">
        <input type="hidden" name="projectGuid" value="${obj.projectGuid}"/>
        <div class="layui-form-item">
            <label class="layui-form-label">项目名称</label>
            <div class="layui-input-block">
                <input type="text" class="layui-input" name="title" lay-verify="title" value="${obj.title}" placeholder="请录入项目名称"/>
            </div>
        </div>
        ...
        <div class="layui-form-item">
            ...
            <div class="layui-input-block">
                <input type="text" name="onlineDate" id="scheduleDate" placeholder="请选择日期" class="layui-input" value="${obj.onlineDateTxt}" lay-verify="scheduleDate" readonly/>
            </div>
        </div>
        ...
        <div class="layui-form-item">
            ...
            <div class="layui-input-block">
                <div class="layui-input-inline">
                    <input type="radio" name="state" value="0" title="关闭" ${obj.state == 0 ? 'checked' : ''}/>
                </div>
                <div class="layui-input-inline">
                    <input type="radio" name="state" value="1" title="激活" ${obj.state == 1 ? 'checked' : ''}/>
                </div>
            </div>
        </div>
        ...
        <div class="layui-form-item">
            ...
            <div class="layui-input-block">
                <input type="text" name="orderNumber" class="layui-input" placeholder="请录入排序序号" value="${obj.orderNumber}" lay-verify="number|orderNumber"/>
            </div>
        </div>
...
<script language="JavaScript">
    ...
    layui.use(['element','form','jquery','laydate','formSelects'],function () {
        var element = layui.element,form = layui.form,$ = layui.jquery,laydate = layui.laydate,
            formSelects = layui.formSelects,user = [];
        <c:forEach var="projectUser" items="${obj.projectUsers}">
        user.push(${projectUser.userId})
        </c:forEach>
        ...
        $.ajax({
            ...
            success:function (res) {
                ...
                formSelects.value('follower', user);
            },
            error:function (kj) {
        ...
        form.on("submit(update)",function (data) {
            ...
            $.ajax({
                ...
                success:function (res) {
                    if(res.code === 1){
                        layer.confirm("成功修改项目，返回上一页?",{btn:['确定','返回'],offset:'10px',anim:4},function () {
