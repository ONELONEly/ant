...
<div class="x-body">
    ...
    <table class="layui-table" lay-data="{url:'./queryAllDept',page:true,limit:10,limits:[10,15,20,25,30,50],id:'department'}" lay-filter="department">
        <thead>
        <tr>
            <th lay-data="{checkbox:true,width:50,fixed:true}"></th>
            <th lay-data="{field:'dept',align:'center',width:'30%',sort:true}">部门编号</th>
            <th lay-data="{field:'dsca',edit:true,align:'center',width:'30%'}">描述</th>
            <th lay-data="{field:'comp',align:'center',width:'10%'-50,sort:true}">部门编号</th>
...
<script language="JavaScript">
    layui.use(["laydate","laypage","element","layer","table","jquery","form"],function () {
        ...
        form.verify({
            dept:function (value) {
                if(checkForm(value) || value.length !== 6){
                    return "ID不为空且长度为6";
                }
            },
            dsca:function (value) {
